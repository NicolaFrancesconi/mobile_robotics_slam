...
class GraphHandler:
    ...
    def add_matched_reflectors_edge(self, matched_indices, pose, landmarks, pose_id, mapped_reflectors, extracted_reflectors):
        for idx_E, idx_M in matched_indices:
            ...
            # Precompute trigonometric terms once
            cos_theta = np.cos(pose[2])
            sin_theta = np.sin(pose[2])
            # Translate and rotate directly
            dx, dy = position - pose[:2]
            relative_position = np.array([cos_theta * dx + sin_theta * dy, -sin_theta * dx + cos_theta * dy])
            self.graph_optimizer.add_pose_landmark_edge_2D(pose_id, id_M, relative_position,  self.laser_information_matrix)
    ...
    def add_non_matched_corners(self, extracted_corners, pose_id, landmarks, non_matched_indices, mapped_corners):
        for idx in non_matched_indices:
            ...
            if np.min(distances) > params.CORNER_ADD_DISTANCE_THRESHOLD:
                self.graph_optimizer.add_landmark_vertex_2D(id, position)
                # Add Edge Between Pose and Landmark
                self.graph_optimizer.add_pose_landmark_edge_from_state_2D(pose_id, id, self.laser_information_matrix)
    ...
    def add_to_graph(self, pose, pointcloud, landmarks):
        ...
        if not match_ref and not match_cor and len(self.pose_vertices) > 0:
            self.odometry_information_matrix = np.diag([10, 10, 300])
    ...
    def add_pose_vertex(self, pose, pointcloud, pose_id):
        id = pose_id
        if pose_id == self.pose_base_id: # First Pose
            self.pose_vertices.append(PoseVertex(id, pose[0], pose[1], pose[2], pointcloud))
            self.graph_optimizer.add_pose_vertex_2D(id, pose, fixed=True)
        else:
            self.pose_vertices.append(PoseVertex(id, pose[0], pose[1], pose[2], pointcloud))
            self.graph_optimizer.add_pose_vertex_2D(id, pose, fixed=False)
            self.graph_optimizer.add_odometry_edge_2D(id-1, id, self.odometry_information_matrix)
            if len(self.pose_vertices) > self.fixed_lag and self.fixed_lag > 0:
                self.graph_optimizer.fix_pose_vertex_2D(id-self.fixed_lag)
